...
const MaterialReviewContent = ({ materialId }: MaterialReviewContentProps) => {
  ...
  const loadMoreReviews = (materialId: string, sort: string) => {
    if (currentPage < totalPages) {
      setCurrentPage((prevPage) => prevPage + 1);
      getReviews(materialId, sort);
    }
  };

  useEffect(() => {
    getReviews(materialId, filter);
  }, [filter, materialId]);
  return (
            ...
            <div className="flex justify-center">
              {totalPages > 1 && (
                <button
                  ...
                  onClick={() => loadMoreReviews(materialId, filter)}
                >
                  <Image
